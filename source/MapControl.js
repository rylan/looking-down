/**
	This comes from the EnyoJS Showcase http://enyojs.com/samples/maps/
*/
...
enyo.kind({
	...
	//* @public
	createPushpin: function(inLatitude, inLongitude, inOptions, inProps, inClick) {
		return this.updatePushpin(null, inLatitude, inLongitude, inOptions, inProps);
	},
	updatePushpin: function(inPushpin, inLatitude, inLongitude, inOptions, inProps, inClick) {
	...
	/* ADDED TO
	*/
	showCurrentPosition:function(inCoord, inPushpinImage, imgWidth, imgHeight)
	{ 
		var w=22, h=39;
		...
		if(inPushpinImage){
			return this.updatePushpin(this.currentLocationPin, inCoord.latitude, inCoord.longitude,
				{icon: inPushpinImage, height: h, width: w, anchor: new Microsoft.Maps.Point(24, 24)});
		} else {
			return this.updatePushpin(this.currentLocationPin, inCoord.latitude, inCoord.longitude,
				{icon: "assets/images/mylocation.png", height: 48, width: 48, anchor: new Microsoft.Maps.Point(24, 24)});
		}
	},
	updateMapCameras: function(inCameras, inExclude){
		var i, 
			p;
		if(inCameras) {
			for(i=0; i < inCameras.length; i+=1 ) {
				if(inExclude && inExclude.src === inCameras[i].src) {
					continue;
				} else {
					try {
						p = this.createPushpin(inCameras[i].geo.latitude, inCameras[i].geo.longitude, {icon: "assets/images/pushpin.png", width: 22, height: 39, anchor: new Microsoft.Maps.Point(24, 24)}, {camera: inCameras[i], });
						Microsoft.Maps.Events.addHandler(p, 'click', enyo.bind(this, this.doCameraPinClick));  
					} catch(e){}
				}
			}
		}
